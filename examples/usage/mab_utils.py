...
def nmr_read(file):
    ...
    with open(file, "rb") as f:
        ...
        while True:
            binary_data = f.read(
                4
            )  # the NMR data is stored as 4-byte real numbers. Read the 4 bytes
            if not binary_data:
                break  # If reach EOF, break out of the loop
...
def exptype(df):
    ...
    for s in df.TITLE:
        base_type = s[
            s.find("-") + 5 : s.find("-") + 8
        ]  # This is the portion of the title containing the NMR frequency
        gross_type = "600"
        if base_type.startswith("8") or base_type.startswith("9"):
            gross_type = "800"
        if base_type.startswith("7"):
            gross_type = "700"
        types += [gross_type]
    return types
...
def read_all_metadata(
    ...
    for column_name, column_data in zip(
        column_names, parsed_data
    ):  # Zip ensures that there will be no mismatched lengths
...
def _read_all_metadata(metadata_file=None, raw_data_path=None):
    ...
    metadata_table = pd.DataFrame(
        columns=df.columns.values[0].split()[1:16], data=parsed_data
    )
...
def save_outlier_data(
    metadata_table, outlier_project, indices_to_split, split_point, metric
):
    # Experiments will be sorted by group_indices; measurements with the same group_indices will be comparable
    # lower_indices are to ensure that data are sorted by a unique identifier
    group_indices = indices_to_split[:split_point]  # ['ExpCode','ExpType']
    ...
    for name, data in metadata_table.groupby(group_indices):
        ...
        for this_group in outlier_project.experiment_groups:
            if this_group.name == description:
                break
...
seismic_with_alpha = make_diverging_colormap(
    "seismic_with_black", crush_point=0.05, mid_color=(1, 1, 1, 1)
)
RdBu_with_black = make_diverging_colormap(
    "RdBu_with_black",
    crush_point=0.05,
    min_color=(0, 0, 1),
...
RdBu_with_white = make_diverging_colormap(
    "RdBu_with_white",
    crush_point=0.05,
    min_color=(0, 0, 1),
...
def make_grid_plot(
    ...
    **subplots_args,
    ...
    fig, axes = plt.subplots(
        numrows, numcols, figsize=fs, squeeze=False, **subplots_args
    )
...
def better_mark_inset(big_ax, inset_ax, corner_1, corner_2, inset_spine_color):
    ...
    rect = mplp.Rectangle(
        ...
        inset_width,
        inset_height,
        edgecolor=inset_spine_color,
        facecolor="none",
        ls=":",
    )
    big_ax.add_artist(rect)
    big_ax.figure.canvas.draw()
    corner_of_axis = big_ax.transData.inverted().transform(
        inset_ax.transAxes.transform(corner_1)
    )
    corner_of_data = inset_ax.transData.inverted().transform(
        inset_ax.transAxes.transform(corner_1)
    )
    big_ax.plot(
        (corner_of_axis[0], corner_of_data[0]),
        (corner_of_axis[1], corner_of_data[1]),
        ls=":",
        color=inset_spine_color,
    )

    corner_of_axis = big_ax.transData.inverted().transform(
        inset_ax.transAxes.transform(corner_2)
    )
    corner_of_data = inset_ax.transData.inverted().transform(
        inset_ax.transAxes.transform(corner_2)
    )
...
def nmr_color_scaled(
    ...
    cb = nmr_annotate(
        im,
        ax,
        cb_ticks=np.array(interp_map[1])[[0, 1, 3, 4]],
