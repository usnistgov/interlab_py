...
def make_grid_plot(
    ...
    **subplots_args,
    ...
    fig.subplots_adjust(
        left=left_buffer, right=1 - right_buffer, top=1, bottom=bottom_buffer
    )
...
class LogMidpointNormalize(mcolors.Normalize):
    def __init__(
        self, vmin=None, vmax=None, midpoint=None, clip=False, linthresh=1, linscale=1
    ):
        mcolors.Normalize.__init__(self, vmin, vmax, clip)
    ...
    def __call__(self, value, clip=None):
        ...
        y[log_positive] = (
            (np.log(value[log_positive]) - self.logthresh) / (self.dec_up)
        ) * (0.5 - self.linscale) + (0.5 + self.linscale)
        y[log_negative] = (
            1 - (np.log(-1 * value[log_negative]) - self.logthresh) / (self.dec_dn)
        ) * (0.5 - self.linscale)

        y[linear] = (
            (value[linear] + self.linthresh) / (self.linthresh) - 1
        ) * self.linscale + 0.5
...
seismic_with_alpha = make_diverging_colormap(
    "seismic_with_alpha", crush_point=0.05, mid_color=(1, 1, 1, 1)
)
...
greys_with_alpha = mcolors.LinearSegmentedColormap.from_list(
    "greys_with_alpha", [(1, 1, 1, 0), (0, 0, 0, 1)]
)
