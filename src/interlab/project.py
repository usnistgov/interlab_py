...
class Project(object):
    ...
    def __init__(
        ...
        self._create_groups(
            xdata=self._x_data, data=data, rawdata=rawdata, data_names=Data_set_names
        )
    ...
    def __str__(self):
        ...
        inls_rep = ["Interlab arrays:"] + [
            str(self.interlab_arrays[metric["metric"]])
    ...
    def plot_distance_fig(
        ...
        * For each metric, a heat map plot of the interspectral distance matrix
        ...
        for metric_num, metric_name in enumerate(distance_metrics):
            ...
            for row in saxes:
                plot_locator = (
                    metric_num + int(plot_data)
                )  # need to increment this by 1 if the data is being plotted with the heat maps
    ...
    def process_mahalanobis(self, sets_to_extract=None, threshold=1e-1):
        ...
        for group in groups_to_use:
            group["Mahalanobis"].mahalanobis_dict = dict(
                VI=VI,
            )
    ...
    def _extract_matrix(self, sets=None, metric=None, screen_outliers=True):
        ...
        if metric is None:
            raise ValueError("Must specify the metric to use")
        ...
        for group in sets_to_extract:
            ...
            if screen_outliers:
                outliers = ~group[
                    metric
                ].outlier_mask  # outlier_mask MASKS the outliers, we want the opposite
            else:
            ...
            for outlier, zscore, data_name in zip(outliers, zscores, data_names):
                ...
                # Otherwise, we've filled it, so take the average
                    ...
                    if not outlier:
                        zscores_array[key_location, lab_location] = (
                            zscores_array[key_location, lab_location] + zscore
                        ) / 2
        return zscores_array

    def extract_experimental_matrix(
        self,
        sets=None,
        metric=None,
        ...
        zscores_array = self._extract_matrix(
            sets=sets, metric=metric, screen_outliers=screen_outliers
        )
        ...
        self.interlab_arrays[metric].lablist = list(
            np.array(self.lablist)[~no_exp_for_lab]
        )
    ...
    def plot_zscores_heatmap(
        ...
        **subplot_kwargs,
        ...
        zscores_array = self._extract_matrix(
            sets=sets, metric=metric, screen_outliers=False
        )
    ...
    def _standard_plot(
        ...
        fig, ax_array = plu.make_grid_plot(
            numrows, numcols, plotsize=(width, height), gridspec_kw=gs_kw, **kwargs
        )
    ...
    def plot_histograms(
        ...
        pdffig, pdfax = self._standard_plot(
            numplots,
            numcols,
            gs_kw=gs_kw,
            xlabel_buffer=xlabel_buffer,
        ...
        for ax, group in zip(pdfax, sets_to_plot):
            group.histogram(
                metric, ax, num_bins=num_bins, max_bin=max_bin, min_bin=min_bin
            )

            # Count number of non-outlier samples
            num_samples = (
                np.count_nonzero(group[metric].outlier_mask),
                len(group.data_names),
            )
    ...
    def plot_zscore_distances(
        ...
        olm0 = sets_to_plot[0][
            metric
        ].outlier_mask.shape  # Get the shape of the outlier mask of the first group
        ...
        zscorefig, zscoreax = self._standard_plot(
            numplots,
            numcols,
            gs_kw=gs_kw,
            xlabel_buffer=xlabel_buffer,
        ...
        for ax, group in zip(zscoreax, sets_to_plot):
            ...
            ax.text(
                0.98, 1.02, group.name, ha="right", va="bottom", transform=ax.transAxes
            )
            ax.yaxis.set_major_locator(pdf_locator)
            if not sharex:
                ax.set_xticklabels([])
    ...
    def plot_projected_zscores(
        ...
        zscorefig, zscoreax_array = plu.make_grid_plot(
            numrows,
            numcols,
    ...
    def plot_zscore_loadings(self, distance_metrics=None, xlabel_buffer=1):
        ...
        loadfig, loadax_array = plu.make_grid_plot(
            numrows,
            numcols,
            plotsize=(width, height),
            xlabel_buffer=xlabel_buffer,
    ...
    def plot_zscore_outliers(self, metric, y_component=1, text=True):
        ...
        if text:
            zscore_outliers_fig.text(
                0.85, 0.8, metric, ha="right", va="top", size=15, transform=ax.transAxes
            )
