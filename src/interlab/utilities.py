...
def idfunc(*args, **kwargs):
    ...
    if len(args) == 1:
        return args[
            0
        ]  # We only passed one argument to the function. We want to return arg, not (arg)
    return args
...
class ExperimentGroup(object):
    ...
    def plot_data(self, ax, linecolor="k", **kwargs):
        ...
        if self.xdata is not None:
            pl = ax.plot(
                self.xdata,
                self.data.T / self.data.max(axis=1)
                + np.array(range(numsets))
                - 0.5 / float(1),
                linecolor,
            )
    ...
    def distance_measure_plot(
        ...
        for metric, ax in zip(metrics_to_plot, ax_dist):
            im = metric.distance_plot(ax, show_titles=show_titles, **distance_kwargs)

            cb = plt.colorbar(im, ax=ax, fraction=0.2)
            cb.locator = plt.MaxNLocator(
                nbins=4, prune="lower", integer=True, min_n_ticks=3
            )
            cb.update_ticks()
...
class DistanceMetric(object):
    ...
    def find_outliers(self, recursive=False, **kwargs):
        ...
        if recursive:
            self.distance_min, self.distance_max = calculate_minmax(
                self.values[self.outlier_mask]
            )
        return
...
class Population(object):
    ...
    def find_outliers(self, recursive=False, support_fraction=0.6, final_screen=False):
        ...
        for i in range(max_num_outliers):
            ...
            if zmax < zlimit:
                break  # If the biggest z-score is within the zlimit, break from the loop
    ...
    def histogram(self, ax, num_bins=20, max_bin=None, min_bin=None, xvals=None):
        ...
        if max_bin is None:
            min_bin, max_bin = self.minmax()
        if xvals is None:
            xvals = np.linspace(min_bin, max_bin, num_bins * 10)

        binsize = (max_bin - min_bin) / float(num_bins)

        hist_norm, bins = np.histogram(
            self.values, range=(min_bin, max_bin), bins=num_bins, density=True
        )
    ...
    def plot_zscores(self, ax, rotation=None):
        ...
        for int_num, (zscore, value, inlier) in enumerate(
            zip(self.zscores, self.values, self.outlier_mask)
        ):
            if inlier:
...
class InterlabArray(object):
    ...
    def __init__(
        ...
        self.population = Population(
            "Projected statistical distance", distribution=self.distribution_function
        )
    ...
    def fit_transform(self):
        ...
        effective_zero = self.pca.transform(
            np.zeros_like(self.pca.mean_.reshape(1, -1))
        )
        ...
        self.active_components[1] = (
            True  # Obviously there should be at least one active component
        )
        ...
        self.population.values = np.linalg.norm(
            self.pca_scores[:, 0:num_active_components], axis=1
        )
    ...
    def plot_components(self, ax):
        ...
        positions = np.arange(
            -total_width, total_width, 2 * width
        )  # list of offsets for all components
        ...
        for component_num in range(num_active_components):
            ...
            edge_intensity = (
                1 - self.pca.explained_variance_ratio_[component_num]
            ) / 1.3
            ...
            im = ax.bar(
                np.arange(len(self.datasets)) + position,
                self.pca.components_[component_num, :] * PCAssign,
                2 * width,
                alpha=1,
                color=bar_colors,
        ...
        ax.set_xlim(
            -1, numspecs + 2
        )  # We'll want 2 labels for the explained variance ratios
        ...
        for component_number in range(num_active_components):
            ...
            component_horizontal_position = len(self.datasets) / (
                len(self.datasets) + 1
            )
            componentscore = "%4.1f" % (
                self.pca.explained_variance_ratio_[component_number] * 100
            )
            componentscore_string = componentscore + " %"
            ax.text(
                component_horizontal_position,
                component_label_position,
                componentscore_string,
                ha="center",
                va="center",
                transform=ax.transAxes,
            )
    ...
    def plot_outliers(self, ax, y_component=1):
        ...
        scatterplot = ax.scatter(
            self.pca_scores[np.invert(z_mask), 0],
        ...
        ctr1 = ax.contourf(
            xx, yy, pdf_for_plot, extend="min", cmap="Greys_r", levels=levelsf, zorder=1
        )
        ctr2 = ax.contour(
            xx,
            yy,
        ...
        component2score = "%5.2f" % (
            self.pca.explained_variance_ratio_[y_component] * 100
        )
        ...
        for j, labeltext in enumerate(self.lablist):
            if not z_mask[j]:
                ax.text(
                    self.pca_scores[j, 0] + 1.0,
                    self.pca_scores[j, y_component],
                    labeltext,
                    ha="left",
                    va="center",
                    size="9",
